...
// Função para calcular possibilidades baseado no número de caracteres n
function calculatePossibilities(n) {
    // Número total de combinações possíveis usando 47 caracteres
    const total = Math.pow(47, n);
    // Probabilidade de acertar uma combinação específica de primeira, em porcentagem com 6 casas decimais
    const probability = (1 / total) * 100;
    // Exibir os dois resultados no console
    console.log(`Número total de combinações possíveis: ${total}`);
    console.log(`Probabilidade de acertar uma combinação específica de primeira: ${probability.toFixed(6)}%`);
...
function encryptText() {
    ...
    // Calcula possibilidades baseado no número de caracteres n
    const n = inputText.length;
    const { total, probability } = calculatePossibilities(n);
    ...
    numPossibilitiesSpan.textContent = total;
    chancePercentSpan.textContent = probability.toFixed(6) + '%';
    // Remove classes de cor anteriores
    chancePercentSpan.className = 'chance-percent low'; // Sempre low para probabilidades pequenas
